...
exports.run = (client, message, args) => {
  ...
  if (reason.length < 1) return message.send.channel('Uyarı sebebini yazmalısın.');
  if (message.mentions.users.size < 1) return message.send.channel('Kimi uyaracağını yazmalısın.').catch(console.error);
  client.users.get(message.author.id).send(reason +' __NEDENİ İLE UYARILDINIZ__')
